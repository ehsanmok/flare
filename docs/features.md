...
| `HttpServer.bind_tls(addr, cert, key, alpn)` + `serve_tls[H](handler)` — in-process HTTPS (TLS-terminated HTTP/1.1) server; no reverse proxy needed. Non-blocking handshake + `SSL_read`/`SSL_write` on `TlsConnHandle`, reusing the exact plaintext parse/serialise; buffered **and** `stream_response` chunked bodies both ride ciphertext | [`https_server.mojo`](../examples/advanced/https_server.mojo), `flare.http.server`, `flare.http._reactor.tls_conn_handle` |

...
| `HttpClient.send_chunked(req, source)` / `send_chunked(method, url, source)` — one-shot streaming upload over H1 chunks or H2/H3 DATA, with a buffered response; no retries, redirects, or pooling | [`test_client_stream_upload_protocols.mojo`](../tests/http/test_client_stream_upload_protocols.mojo), [`test_h3_live_dial.mojo`](../tests/http/test_h3_live_dial.mojo) |
| `HttpClient.send_streaming(req)` / `get_streaming(url)` / HTTPS-only `get_streaming_tls(url)` `-> HttpStreamResponse` — arbitrary buffered request, bounded response reads over H1/H2/H3, final headers available immediately, trailers at EOS, explicit close, and limited collection. Defaults to identity encoding; explicit compressed responses remain raw. Redirects/retries apply only before returning the response | [`streaming design`](superpowers/specs/2026-08-11-http-client-streaming-h2-h3-design.md), [`test_download_decoder.mojo`](../tests/http/test_download_decoder.mojo), [`test_h2_streaming_state.mojo`](../tests/http2/test_h2_streaming_state.mojo), [`test_h3_response_reader.mojo`](../tests/h3/test_h3_response_reader.mojo), [`test_client_stream_response.mojo`](../tests/http/test_client_stream_response.mojo) |
...
| `Http2Config` — validated SETTINGS knobs and a buffered request-body limit (`max_body_size`, default 10 MiB); oversized bodies reset only their stream | [`http2_config.mojo`](../examples/advanced/http2_config.mojo) |

...
| Sanitised error responses | 4xx / 5xx bodies do not echo extractor `raise Error(...)` messages by default. Switch with `ServerConfig.expose_error_messages = True` for local dev. | none in prod | v0.5.0 Step 1 |
...
| Per-handler / per-body / per-request deadlines | `ServerConfig.{handler_timeout_ms, read_body_timeout_ms, request_timeout_ms}`. Earliest fires first; flips `Cancel` to `TIMEOUT`. | none in prod | v0.5.0 Step 1 |
| Graceful shutdown | `HttpServer.drain(timeout_ms)` closes the listener, flips `Cancel.SHUTDOWN`, waits for in-flight handlers, hard-closes at the deadline. Returns a `ShutdownReport`. | handle `Cancel.SHUTDOWN` if you want graceful exit; otherwise the deadline does the right thing. | v0.5.0 Step 1 |
| SIGTERM / SIGINT | `install_drain_on_sigterm[srv](timeout_ms=30_000)` wires up a libc `sigaction` to call `drain` before exit. Container deployments get the standard 30 s grace period for free. | call once at startup if you want it | v0.5.0 Step 1 |
| Multicore drain coordination | `Scheduler.drain(timeout_ms)` broadcasts to every worker; returns one `ShutdownReport` per worker (in-flight at deadline, drained, timed-out). | none | v0.5.0 Step 1 |

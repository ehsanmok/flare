...
| Multicore drain coordination | `Scheduler.drain(timeout_ms) -> List[ShutdownReport]` broadcasts to every worker, closes each worker's listener, joins. Returns one report per worker; counts are best-effort (workers join cooperatively when they observe the stopping flag on their next reactor poll, ~100ms cycle). The reactor-flip-on-shutdown variant — flipping `Cancel.SHUTDOWN` on every in-flight `ConnHandle` so handlers short-circuit — needs the worker-side per-conn registry exposure that lands as a follow-up. | call `Scheduler.drain(timeout_ms)` from the main thread (or via the SIGTERM helper once that lands) | shipped v0.5.0 Step 2 (cancel-flip follow-up) |
| Streaming response bodies | `Body` / `ChunkSource` / `InlineBody` / `ChunkedBody[Source]` shipped (`flare/http/body.mojo`). `ChunkedBody.content_length()` returns `None` so the reactor will use `Transfer-Encoding: chunked` once it adopts the pull loop. The reactor-side write loop (pulling chunks on writable edges) and `HttpServer.serve_streaming` entry point land as a follow-up; until then `ChunkSource` is usable end-to-end via the `drain_body` helper but the wire path keeps the v0.4.x "buffer the whole response before send" shape. | until reactor adoption lands, keep response bodies bounded | shipped v0.5.0 Step 2 (reactor adoption follow-up) |
...
| `RequestView[origin]` for borrowed bodies (zero-copy reads) | `RequestView[origin]` + `parse_request_view` shipped (`flare/http/request_view.mojo`). View borrows method-token / URL / headers / body from a single byte buffer (typically `ConnHandle.read_buf`) via offsets — `view.body()` returns a `Span[UInt8, origin]` directly without copying. `view.into_owned() -> Request` for handlers that need owned state. The reactor's read path still calls `_parse_http_request_bytes` and constructs an owned Request; switching that to `parse_request_view` (with a `ViewHandler` trait carrying `serve_view(req: RequestView[origin])`) is the integration step that flips the "no allocation per request on the read path" gate green; lands as a follow-up alongside the streaming-body work. | none | shipped v0.5.0 Step 2 (integration follow-up) |

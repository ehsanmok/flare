...
The full driver -- per-stream :class:`flare.http3.Http3RequestReader`
-> :trait:`Handler` -> :func:`flare.http3.encode_response_headers`
...
def test_stream_type_codepoints() raises:
    ...
    assert_equal(Http3StreamType.CONTROL, 0x00)
    assert_equal(Http3StreamType.PUSH, 0x01)
    assert_equal(Http3StreamType.QPACK_ENCODER, 0x02)
    assert_equal(Http3StreamType.QPACK_DECODER, 0x03)


def test_config_defaults() raises:
    """:class:`Http3Config` defaults match the production
...
def test_h3_connection_defaults() raises:
    """A fresh :class:`Http3Connection` carries the default config
...
def test_with_config_overrides() raises:
    """:meth:`Http3Connection.with_config` copies in a non-default

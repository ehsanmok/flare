...
:class:`flare.http.HttpClient`: the ``prefer_http3`` knob (the
constructor kwarg or the :meth:`with_prefer_http3` builder), the
``Alt-Svc`` discovery cache
(:meth:`record_alt_svc`), and the per-request decision
(:meth:`http3_wire_choice`). No network I/O -- the policy is pure over
...
def test_default_no_h3() raises:
    ...
    assert_equal(
        c.http3_wire_choice(
            String("https"), String("example.com"), UInt16(443)
        ),
        Http3WireChoice.HTTP_2_OR_LOWER,
    )


def test_prefer_http3_knob() raises:
    var c = HttpClient().with_prefer_http3()
    assert_equal(
        c.http3_wire_choice(
            String("https"), String("example.com"), UInt16(443)
        ),
    ...
    assert_equal(
        c.http3_wire_choice(String("http"), String("example.com"), UInt16(80)),
        Http3WireChoice.HTTP_2_OR_LOWER,
    )


def test_prefer_http3_ctor_kwarg() raises:
    # The constructor kwarg is the ergonomic equivalent of the
    # with_prefer_http3() builder (mirrors prefer_h2c): same wire choice,
    # cleartext still never rides h3.
    var c = HttpClient(String("https://example.com"), prefer_http3=True)
    assert_equal(
        c.http3_wire_choice(
            String("https"), String("example.com"), UInt16(443)
        ),
...
def test_alt_svc_advert_upgrades() raises:
    ...
    assert_equal(
        c.http3_wire_choice(String("https"), String("other.com"), UInt16(443)),
        Http3WireChoice.HTTP_2_OR_LOWER,
    )
...
def main() raises:
    test_default_no_h3()
    test_prefer_http3_knob()
    test_prefer_http3_ctor_kwarg()

...
- **Production hygiene**: per-request `Cancel` token (peer FIN, timeout, drain unified), server-side TLS with cert reload + mTLS + ALPN, streaming bodies with backpressure, sanitised 4xx/5xx, graceful drain with per-worker `ShutdownReport`s, 24 h soak harness, **22 fuzz harnesses (5M+ runs, zero known crashes)**.
- **v0.7 networking + framework parity** (additive, no v0.6 break): HAProxy PROXY protocol v1 + v2 parser (`flare.http.parse_proxy_protocol`, fuzz-clean over 200K runs); `UnixListener` / `UnixStream` AF_UNIX subsystem under `flare.uds` for sidecar IPC; first-class Server-Sent Events surface (`SseChannel`, `SseEvent`, `sse_response`, `SseStreamingResponse`); RFC 9110 §13 `Conditional[Inner]` middleware for `If-Match` / `If-None-Match` / `If-Modified-Since` / `If-Unmodified-Since` (304 / 412 emission); HTTP `RedirectPolicy` (`FOLLOW_ALL` / `SAME_ORIGIN_ONLY` / `DENY`); JSON-per-line `StructuredLogger[Inner]` for Datadog / Loki / Splunk; `Metrics[Inner]` Prometheus text exposition (counter / histogram / gauge); server-side `BearerExtract` / `BasicExtract` + double-submit-cookie `CsrfToken` constant-time comparator; `RequestChunkSource` ChunkSource for inbound bodies (handler-side surface for upcoming reactor streaming); askama-shape `Template` engine (`{{ var }}`, `{% if %}`, `{% for %}`, OWASP HTML escape).

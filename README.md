...
**HTTP/3 throughput (reactor I/O loop pending):** The flare h3 codec, AEAD, rustls QUIC binding, state machine, and H3 dispatch landed in v0.8. The live UDP reactor I/O loop is the one wire path still pending. At HEAD `65b3282`: `quiche 0.22` runs `~72.5k req/s` clean; `quinn 0.11 + h3 0.0.8` errors at the 100-stream workload and needs calibration; flare h3 awaiting the reactor. Full table, baselines, and h2load+H3 build recipe in [`docs/benchmark.md#http3-throughput`](docs/benchmark.md#http3-throughput).
